...
enum TileState { zakryte, bomby, otwarte, flagi, odkryte }
...
class SaperBoard extends StatefulWidget {
  @override
  BoardState createState() => BoardState();
}

class BoardState extends State<SaperBoard> {
  final int rows = 9;
  final int cols = 9;
  ...
  void resetBoard() {
    ...
    uiState = new List<List<TileState>>.generate(rows, (row) {
      return new List<TileState>.filled(cols, TileState.zakryte);
    });
  ...
  Widget buildBoard() {
    ...
    for (int y = 0; y < rows; y++) {
      ...
      for (int x = 0; x < cols; x++) {
        ...
        if (!alive) {
          if (state != TileState.bomby)
            state = tiles[y][x] ? TileState.odkryte : state;
        }

        if (state == TileState.zakryte || state == TileState.flagi) {
          rowChildren.add(GestureDetector(
            onLongPress: () {
              flag(x, y);
            },
            onTap: () {
              if (state == TileState.zakryte) probe(x, y);
            },
            child: Listener(
                child: CoveredMineTile(
              flagged: state == TileState.flagi,
              posX: x,
              posY: y,
            )),
          ));
          if (state == TileState.zakryte) {
            hasCoveredCell = true;
          }
        } else {
            ...
            child: Row(children: <Widget>[
              ...
              Container(
                ...
                child: RichText(
                  text: TextSpan(
                      text: wonGame
                          ? "Wygrales! Zajeło ci to $timeElapsed sekund"
                          : alive
                              ? "Znalezione bomby: $minesFound Wszystkie bomby: $numOfMines $timeElapsed sekund"
                              : "Przegrales! Zajęło ci to $timeElapsed sekund"),
                ),
              ),
            ]),
          )),
  ...
  void probe(int x, int y) {
    if (!alive) return;
    if (uiState[y][x] == TileState.flagi) return;
    setState(() {
      if (tiles[y][x]) {
        uiState[y][x] = TileState.bomby;
        alive = false;
        timer.cancel();
        stopwatch.stop();
      } else {
  ...
  void open(int x, int y) {
    ...
    if (uiState[y][x] == TileState.otwarte) return;
    uiState[y][x] = TileState.otwarte;
  ...
  void flag(int x, int y) {
    if (!alive) return;
    setState(() {
      if (uiState[y][x] == TileState.flagi) {
        uiState[y][x] = TileState.zakryte;
        --minesFound;
      } else {
        uiState[y][x] = TileState.flagi;
        ++minesFound;
      }
    });
  }
